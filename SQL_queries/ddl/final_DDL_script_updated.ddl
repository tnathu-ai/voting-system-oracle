...
-- Creating a tablespace for voter data
CREATE TABLESPACE voters_data
DATAFILE 'voters_data01.dbf'
SIZE 21G;  -- Initial size based on earlier calculation

-- If the space fills up, we can add more space to the tablespace
ALTER TABLESPACE voters_data
ADD DATAFILE 'voters_data02.dbf'
SIZE 3G;  -- Additional space based on our earlier calculation
...
TABLESPACE voter_data
STORAGE (
  INITIAL 21G  -- Initial space allocation for the tablespace: 21 gigabytes.
  NEXT 1G      -- Size of subsequent space allocations after the initial is filled: 1 gigabyte.
  PCTINCREASE 0 -- Ensures all extents after the initial one remain at a consistent size of 1 gigabyte.
);
...
ALTER TABLE voter
PARTITION BY LIST (residentialaddrstate) (
    PARTITION p_NSW VALUES ('NSW'),
    PARTITION p_VIC VALUES ('VIC'),
    PARTITION p_QLD VALUES ('QLD'),
    PARTITION p_SA VALUES ('SA'),
    PARTITION p_WA VALUES ('WA'),
    PARTITION p_TAS VALUES ('TAS'),
    PARTITION p_ACT VALUES ('ACT'),
    PARTITION p_NT VALUES ('NT')
);
